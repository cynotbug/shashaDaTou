<template>
  ...
  >
    <div
      ...
      v-if="!merchantDetail.userTractId"
    >
...
const forceUpdate = inject("updateFn");
...
const collectOperate = () => {
  addCollect(merchantDetail.value.id);
};
...
const cancelOperate = () => {
  ElMessageBox.confirm("是否确认取消收藏?", "确认", {
    ...
    .then(() => {
      deleteCollect(merchantDetail.value.userTractId);
    })
    .catch(() => {});
};
...
const contentWidth = computed(() => {
  return props.size == "large" ? "196px" : "116px";
});
...
.collect-container {
  .collect-not {
    ...
    &-icon {
      ...
      background-image: url("../../../assets/img/un_collect_icon.png");
      background-size: 100% 100%;
    }
